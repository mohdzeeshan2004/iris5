...
# -------------------- JOINT PLOT --------------------
    ...
    if x_col == y_col:
        ...
        if kind in ["scatter", "kde"]:
            g = sns.jointplot(data=data, x=x_col, y=y_col, hue="species", kind=kind)
        else:
...
# -------------------- BOXEN PLOT --------------------
    ...
    sns.boxenplot(data=data, x="species", y=y_col, ax=ax)
...
# -------------------- STRIP PLOT --------------------
    ...
    fig, ax = plt.subplots(figsize=(8, 5))
    sns.stripplot(data=data, x="species", y=y_col, ax=ax)
    ax.set_title(f"{y_col} by Species")
    st.pyplot(fig, clear_figure=True)
...
# -------------------- SWARM PLOT --------------------
    ...
    fig, ax = plt.subplots(figsize=(8, 5))
    sns.swarmplot(data=data, x="species", y=y_col, ax=ax)
    ax.set_title(f"{y_col} by Species")
    st.pyplot(fig, clear_figure=True)
